...
export default class SwapController extends Controller {
  public async index() {
    ...
    if (
      ...
      res = await service.swapServiceV2.swap(ctx.request.body);
    } else {
      res = await service.swapService.swap(ctx.request.body);
    }
